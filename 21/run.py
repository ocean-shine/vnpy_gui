...
class OrderMonitor(QtWidgets.QTableWidget):
    ...
    def process_order_event(self, event: Event) -> None:
        ...
        if order.vt_orderid not in self.cells:
            ...
            # 使用自定义的单元格
            symbol_cell      : Cell = Cell(str(order.symbol))  
            exchange_cell    : Cell = Cell(str(order.exchange))
            orderid_cell     : Cell = Cell(str(order.orderid)) 
            type_cell        : Cell = Cell(str(order.type))    
            direction_cell    : Cell = Cell(str(order.direction))
            offset_cell      : Cell = Cell(str(order.offset))  
            price_cell       : Cell = Cell(str(order.price))   
            volume_cell      : Cell = Cell(str(order.volume))  
            traded_cell      : Cell = Cell(str(order.traded))  
            status_cell      : Cell = Cell(str(order.status))  
            datetime_cell    : Cell = Cell(str(order.datetime)) 
            reference_cell   : Cell = Cell(str(order.reference)) 
            
            # 设置单元格
            self.setItem(0, 0, symbol_cell)  
            self.setItem(0, 1, exchange_cell)
            self.setItem(0, 2, orderid_cell) 
            self.setItem(0, 3, type_cell)    
            self.setItem(0, 4, direction_cell)
            self.setItem(0, 5, offset_cell)  
            self.setItem(0, 6, price_cell)   
            self.setItem(0, 7, volume_cell)  
            self.setItem(0, 8, traded_cell)  
            self.setItem(0, 9, status_cell)  
            self.setItem(0, 10, datetime_cell) 
            self.setItem(0, 11, reference_cell) 
            
            # 保存单元格 
            self.cells[order.vt_orderid] = {
                "symbol":       symbol_cell,  
                "exchange":     exchange_cell,
                "orderid":      orderid_cell, 
                "type":         type_cell,    
        ...
        # 如果是老的数据
            ...
            cells["symbol"].setText(str(order.symbol))  
            cells["exchange"].setText(str(order.exchange))
            cells["orderid"].setText(str(order.orderid)) 
            cells["type"].setText(str(order.type))    
            cells["direction"].setText(str(order.direction))
            cells["offset"].setText(str(order.offset))  
            cells["price"].setText(str(order.price))   
            cells["volume"].setText(str(order.volume))  
            cells["traded"].setText(str(order.traded))  
            cells["status"].setText(str(order.status))  
            cells["datetime"].setText(str(order.datetime)) 
            cells["reference"].setText(str(order.reference)) 
